...
def _tuned_cooldown_for_duration(duration_seconds: int) -> int:
    """Return conservative cooldown defaults for dynamically discovered modes."""
    ...
    if duration_seconds <= 360:
        return 1260
    return max(1440, int(round(duration_seconds * 3.5)))
...
EXPLAIN_TOPIC_GUIDES = {
    ...
    "schedule": {
        ...
        "flags": ["--schedule PRESET", "--cooldown-scale FACTOR", "--duty-cycle FRACTION", "--min-captures N"],
        "failures": [
            "TX skipped due to cooldown, min-captures, or duty-cycle gating",
            "schedule relies on MMSSTV modes when library is unavailable",
        ],
        "checklist": [
            "Run --list-schedules",
            "Pick preset for ascent/cruise/float phase",
            "Tune cooldown-scale and duty-cycle before flight",
        ],
        "commands": [
            "python3 pi_sstv.py --list-schedules",
            "python3 pi_sstv.py --schedule hab_rapid --total 200 --interval 8",
...
def print_help_cli_overview():
    print(
        ...
        "  Safety : --no-tx, --tx-power, --pd-idle, --cooldown-scale, --duty-cycle, --min-captures\n"
...
def print_mission_summary(runtime_state):
    ...
    log_stage_header(
        ...
        [
            ...
            ("duty", f"{budget_seconds}s / {ROLLING_DUTY_CYCLE_WINDOW_SECONDS}s  ({int(MAX_TRANSMIT_DUTY_CYCLE * 100)}% max)"),
            ("cooldown", f"scale={COOLDOWN_SCALE_FACTOR}x"),
            *describe_radio_control_states(),
            ("ptt_pins", get_active_ptt_pins()),
            ("audio", f"{SLOWFRAME_AUDIO_FORMAT}  {SLOWFRAME_SAMPLE_RATE}Hz  aspect={SLOWFRAME_ASPECT_MODE}"),
            ("alsa", f"playback={ALSA_AUDIO_DEVICE or 'auto-select'}  mixer={describe_alsa_guardrails()}"),
