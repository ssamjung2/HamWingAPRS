...
// VHF Radio Module
#define PIN_V_DRA_RX 0  // VHF DRA RX (SERCOM0.3) Serial1
#define PIN_V_DRA_TX 1  // VHF DRA TX (SERCOM0.2) Serial1
#define V_SQLPin 14     // VHF squelch
#define V_PTTPin 16     // VHF Push-To-Talk (PTT)
// UHF Radio Module
#define PIN_U_DRA_RX 11  // UHF from module TXD via JP1/F4 (SERCOM1.0)
#define PIN_U_DRA_TX 10  // UHF to module RXD via JP2/F3 (SERCOM1.2)
...
//////////////////////////////////////////////////////////////////////////////
// SERCOM (Serial Communication) - Multiplexed Serial Configuration on SAMD21
// Serial1 - VHF
// Built in hardware serial
// Uart Serial1(&sercom0, PIN_V_DRA_RX, PIN_V_DRA_TX, SERCOM_RX_PAD_0, UART_TX_PAD_2);
// void SERCOM1_Handler()
// {
//   Serial2.IrqHandler();
// }
// Serial2 - UHF preferred mapping from KiCad:
// JP1/F4 -> U2 pad17 (module TXD) so MCU RX is D11/PAD0,
// JP2/F3 -> U2 pad16 (module RXD) so MCU TX is D10/PAD2.
...
void ensureSerial2Mux() {
  // Re-assert SERCOM1 mux for D10/D11 in case another init path changed it.
  pinPeripheral(PIN_U_DRA_RX, PIO_SERCOM);  // D11 (PA16) -> SERCOM1 PAD0
  pinPeripheral(PIN_U_DRA_TX, PIO_SERCOM);  // D10 (PA18) -> SERCOM1 PAD2
}
...
bool applyActiveDefaults() {
  ...
  gVhfConfigured = false;
  gUhfConfigured = false;
  bool vhfOk = applyDefaults(F("SERIAL1/VHF PORT"), draV, VHF_DEFAULTS);
...
#else
  gUhfConfigured = false;
  gVhfConfigured = applyDefaults(F("SERIAL1/VHF PORT"), draV, VHF_DEFAULTS);
...
void setup() {
  ...
  Serial1.begin(9600);
  activateUhfUart(true);
  // pinPeripheral MUST be called AFTER begin() — begin() re-applies the
  // board variant's default pin type (PIO_TIMER / PIO_SERCOM_ALT), which
  // would disconnect SERCOM1 from the physical D10/D11 pins.
  NVIC_EnableIRQ(SERCOM1_IRQn);  // Enable SERCOM1 interrupt for UHF RX

  logDebugMessage(F("Startup"), F("Preferred UHF wiring: Feather D11 receives from UHF and Feather D10 transmits to UHF. Shared PD control is on Feather D12."));
#if TRY_UHF_BOTH_UART_MAPPINGS
...
void loop() {
  ...
  if (!gUhfConfigured) {
    ...
    activateUhfUart(true);
    Serial.println(F("[UHF-RETRY] UART re-initialized and SERCOM1 mux re-applied (preferred map)."));
    ...
    if (!preferredOk) {
      ...
      activateUhfUart(false);
      Serial.println(F("[UHF-RETRY] UART re-initialized and SERCOM1 mux re-applied (fallback map)."));
